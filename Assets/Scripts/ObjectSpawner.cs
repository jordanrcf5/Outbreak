...
public class ObjectSpawner : MonoBehaviour
{
    ...
    private ObjectType RandomObjectType()
    {
        ...
        else if (randomChoice <= (enemyProbability + bigGemProbibility))
        {
    ...
    private void SpawnObject()
    {
        ...
        if (validPositionFound)
        {
            ...
            GameObject spawnedObj = Instantiate(objectPrefabs[(int)objectType], spawnPosition, Quaternion.identity);

            // ✅ FIXED: store REAL spawned object
            spawnObjects.Add(spawnedObj);

            //Destroy gems only after time
            if (objectType != ObjectType.Enemy)
            {
                // ✅ FIXED: destroy REAL spawned object
